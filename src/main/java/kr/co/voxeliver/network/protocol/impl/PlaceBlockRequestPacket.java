...
public class PlaceBlockRequestPacket implements Packet {
    private int x;
    private int y;
    private int z;
    private int blockType;

    public PlaceBlockRequestPacket() {
    }

    public PlaceBlockRequestPacket(Vector3 position, int blockType) {
        this((int) Math.floor(position.x), (int) Math.floor(position.y), (int) Math.floor(position.z), blockType);
    }

    public PlaceBlockRequestPacket(int x, int y, int z, int blockType) {
        this.x = x;
        this.y = y;
        this.z = z;
